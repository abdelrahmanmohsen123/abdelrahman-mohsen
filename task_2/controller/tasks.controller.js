...
const updateTask=(req,res)=>{
    ...
    if(Object.keys(req.query).length != 0){
        if(req.query.title == '') data.errors.push('plz,insert title')
        if(req.query.content == '') data.errors.push('plz,insert content')
        if(data.errors.length == 0) {
    const id = req.params.id
    const allTasks = taskHelper.readData()
    let record = allTasks.find(task=> task.id == id )
    ...
    console.log(myInsertedData)
    record.title=myInsertedData.title
    record.content=myInsertedData.content
    record.status=myInsertedData.status
...
module.exports={ addTask, editTask,updateTask, showAll, showSingle, deleteTask }